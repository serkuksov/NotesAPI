...
class UserFilter(Filter):
    user_name__like: Optional[str] = Field(Query(
        alias='user',
        default=None,
...
class NoteFilter(Filter):
    title: Optional[str] = Field(Query(
        default=None,
        description='Фильтр по названию заметки',
    ))
    user: Optional[UserFilter] = FilterDepends(UserFilter)
    order_by: Optional[list[str]] = Field(Query(
        default='-created_at,+title',
        description='Сортировка по дате создания и названию заметки, + в порядке возрастания, - в порядке убывания',
    ))
